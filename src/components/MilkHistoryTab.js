...
import { Calendar, Loader2, Edit3, Trash2, ChevronDown, ChevronRight, Sun, Moon, Check, X } from 'lucide-react'
import Avatar from './Avatar'
import Button from './Button'
import { formatThaiDate } from '../lib/datetime'

export default function MilkHistoryTab({ records, cows, loading, updateRecord, deleteRecord, historyFilters }) {
  ...
  useEffect(() => {
    ...
    // Date filter
    if (historyFilters.selectedDate) {
      filtered = filtered.filter(record => record.date === historyFilters.selectedDate)
    }
    ...
    if (historyFilters.searchTerm) {
      ...
      filtered = filtered.filter(record =>
        record.cowId.name.toLowerCase().includes(searchTerm)
      )
    }
    ...
    <div className="pb-40">
      ...
      {filteredHistory && filteredHistory.length > 0 && (
        (() => {
          ...
          const avgMilk = totalMilk / filteredHistory.length

          // Format date and get day of week
          const selectedDate = historyFilters.selectedDate
          ...
          const formattedDate = `วัน${dayOfWeek}ที่ ${day} ${month} ${year}`
              ...
              <div className="p-4 bg-gradient-to-r from-blue-50/30 via-white/50 to-green-50/30">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="font-light text-lg text-gray-900">{formattedDate}</div>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
      ...
      <div className="bg-white/90 backdrop-blur-xl rounded-3xl border-0 shadow-[0_8px_32px_rgba(0,0,0,0.08)]">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-sm font-medium text-gray-500">ไม่พบข้อมูลที่ตรงกับเงื่อนไข</p>
          </div>
        ) : (
